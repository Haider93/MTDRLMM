...
class Agent():
    def __init__(self, num_actions, num_states_vars, gamma, isModel):
    ...
    def build_network(self):
        #task model
        # task_input = Input(shape=(self.state_vars,))
        # task_hidden = Dense(64, activation='relu')(task_input)
        # task_output = Dense(self.num_actions, activation='linear', name='task_output')(task_hidden)
        # model = Model(inputs=task_input, outputs=task_output, name='task_model')
        # model.compile(loss="mean_squared_error",
        #                optimizer=Adam(lr=0.001))

        model = Sequential()
        model.add(InputLayer(input_shape=(self.state_vars,), name='task_input'))
        model.add(Dense(64))
        ...
        model.add(Dense(self.num_actions, activation='linear', name='task_output'))
        model.compile(loss="mean_squared_error",
                      optimizer=Adam(lr=0.001))

        # model.compile(loss="mean_squared_error",
        #               optimizer=SGD(lr=0.001, momentum=0.7))
    ...
    def save_model(self, model_file, weights_file):
        # save model to json
        model_json = self.model_net.to_json()
        with open(model_file, "w") as json_file:
            json_file.write(model_json)  #
        self.model_net.save_weights(weights_file)
...
#loaded_model = load_model("saved_model.json", "saved_model_weights.h5")
agent = Agent(num_actions=NUM_ACTIONS, num_states_vars=NUM_STATE_VARS, gamma=GAMMA, isModel=False)
agent.save_model("task_model_ivv.json", "task_model_weights_ivv.h5")
...
def train_model(curr_state=None, next_state=None, reward=None,action=None):
    ...
    agent.save_weights("task_model_weights_ivv.h5")
    #self.update_target_network()
